...
#            worksheet = workbook.add_sheet(warehouse_id.name)
            ...
            worksheet[c].write_merge(0, 1, 0, 10, 'STOCK INVENTORY',main_header_style)

            print_date = 'Impreso el : '+ str(datetime.date.today().strftime('%d-%m-%Y'))
            start_date = 'Desde : '
            if self.start_date:
...
#
            ...
            worksheet[c].write_merge(3, 3, 7,10, 'INVENTARIO VALUADO - COSTO',second_header_cus_nubold)
            worksheet[c].write(4,4, 'Compras ',second_header_cus_nubold)
            worksheet[c].write(4,5, 'Ventas',second_header_cus_nubold)
            worksheet[c].write(4,6, 'Final',second_header_cus_nubold)
            worksheet[c].write(4,7, 'C/U',second_header_cus_nubold)
            worksheet[c].write(4,8, 'Compras ',second_header_cus_nubold)
            worksheet[c].write(4,9, 'Ventas ',second_header_cus_nubold)
            worksheet[c].write(4,10, 'Final  ',second_header_cus_nubold)

            worksheet[c].write(4,0, 'Codigo ',second_header_cus_nubold)
            worksheet[c].write(4,1, 'Nro Comprobante',second_header_cus_nubold)
            worksheet[c].write(4,2, 'Fecha',second_header_cus_nubold)
            worksheet[c].write(4,3, 'Cliente',second_header_cus_nubold)
#
            ...
            for location_id in self.location_id:
                for product in self.product_ids:
                    ...
                    if move_ids:
                        move_ids = self.env['stock.move'].search([('id','in',move_ids.ids)],order='date')
                        ...
                        if product_code:
                            pro_name = product_code+ ' - '+pro_name

                        worksheet[c].write_merge(i, i, 1,9, pro_name  ,header_cus_nubold)
                        i+=1
                        worksheet[c].write(i,0,product_code or ' ',text_left)
                        worksheet[c].write(i,1,'Saldo inicial' ,text_left)
                        worksheet[c].write(i,2,date ,text_left)
                        worksheet[c].write(i,3,'Saldo inicial' ,text_left)
...
#                                cu_value = quat_value / in_qty
                                cu_value = product.standard_price
